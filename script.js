...
function gridGen() {
  for (let i = 0; i < 9; i++) {
    grid[i] = [];
    for (let j = 0; j < 9; j++) grid[i].push(0);
  }
  for (let y = 0; y < 9; y++) {
    for (let x = 0; x < 9; x++) {
      if (grid[y][x] == 0) zero_c++;
    }
  }
  //let start, end;
      ...
      if (grid[y][x] == 0) {
        ...
        number_set.forEach((n) => {
          if (possible(y, x, n)) {
            ...
            grid_blank_filled++;
            solve();
            if (zero_c == grid_blank_filled) return;
            grid[y][x] = 0;
            grid_blank_filled--;
          }
        });
...
function check(index) {
  ...
  uc = uc.union(bc);
  dc = gs.difference(uc);
  if (uc.has(n)) boxes[index].style.backgroundColor = "#ffbab0";
  else {
...
function gameLevel() {
  ...
  gamelevel.forEach((el, index) => {
    el.onclick = function () {
      if (el.innerText == "Hard") blank_pos_count = 60;
      else if (el.innerText == "Medium") blank_pos_count = 50;
      else blank_pos_count = 40;
      newGrid();
    };
  });
}
